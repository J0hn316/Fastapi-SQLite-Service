...
router = APIRouter(
    prefix="/notes", tags=["notes"], dependencies=[Depends(require_api_key)]
)
...
def create_note_route(
    ...
    note = create_note(db, payload, actor=actor, request=request)
    return note
...
def update_note_route(
    ...
    if not note:
        raise HTTPException(status_code=404, detail="Note not found")
    return update_note(db, note, payload, actor=actor, request=request)
...
def delete_note_route(
    ...
    if not note:
        raise HTTPException(status_code=404, detail="Note not found")
    delete_note(db, note, actor=actor, request=request)
    return None
